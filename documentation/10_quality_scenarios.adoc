...
=== Quality Scenarios (Product Owner)
...
The following test scenarios aim to test the behavior of the adapter scaling approach.
The target behavior is that all messages arrive reliably in one or more destination queues.
...
|1
|Happy Path1
a|A normal scaling operation, further adapter instances (1 per 10 seconds) are started.
The ... send messages periodically (once every 10 seconds).

. {blank}
.. There is 1 instance at the beginning
.. There are 1000 ...

|2
|Happy Path 2
a|One scaling operation, further devices (10 per minute) must be integrated.
The ... send messages acyclically (Between 1 to 120 seconds).

. {blank}
.. There are 3 instances
.. There are 1000 ... at the beginning

|3
|Minimal Path
a|No new ... (at least 1 ... is present), one message per ... per 10 minutes.
...
|4
|Maximum Path
a|1000 new ... in 10 seconds, 1000 acyclic messages (between 0.1 and 10 second intervals).

. {blank}
.. There is 1 instance at the beginning
.. There are 100 ... at the beginning

|5
|Acyclical Path
|Similar to maximum path: Simulated devices (1-1000) are stopped and removed acyclically.
|===
